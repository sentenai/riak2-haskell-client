...
import Control.Exception (Exception, IOException, throw, throwIO)
import Control.Monad (forM_, replicateM, replicateM_, liftM)
...
throwIOError :: ErrorResponse -> IO a
throwIOError = throwIO
...
getResponse conn len v expected = do
  ...
  case undefined of
   _| tag == expected        -> recvGetN conn (len-1) messageGetM
    | tag == T.ErrorResponse -> throwIOError =<< recvGetN conn (len-1) messageGetM
    | otherwise ->
...
recvCorrectTag func conn expected len v = do
  ...
  case undefined of
   _| tag == expected -> recvExactly conn (len-1) >> return v
    | tag == T.ErrorResponse -> throwIOError =<< recvGetN conn len messageGetM
